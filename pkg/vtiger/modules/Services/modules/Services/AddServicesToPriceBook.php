...
$list_header .= '<td class="lvtCol">'.$mod_strings['LBL_LIST_SERVICE_NAME'].'</td>';
if(getFieldVisibilityPermission('Services', $current_user->id, 'unit_price') == '0')
	$list_header .= '<td class="lvtCol">'.$mod_strings['LBL_SERVICE_UNIT_PRICE'].'</td>';
...
$prod_price_list = getPricesForProducts($currency_id, $new_prod_array, 'Services');

$list_body ='';
for($i=0; $i<count($new_prod_array); $i++)
{
	$log->info("Services :: Showing the List of services to be added in price book");
