<?php
/*********************************************************************************
 * The contents of this file are subject to the SugarCRM Public License Version 1.1.2
 * ("License"); You may not use this file except in compliance with the
 * License. You may obtain a copy of the License at http://www.sugarcrm.com/SPL
 * Software distributed under the License is distributed on an  "AS IS"  basis,
 * WITHOUT WARRANTY OF ANY KIND, either express or implied. See the License for
 * the specific language governing rights and limitations under the License.
 * The Original Code is:  SugarCRM Open Source
 * The Initial Developer of the Original Code is SugarCRM, Inc.
 * Portions created by SugarCRM are Copyright (C) SugarCRM, Inc.;
 * All Rights Reserved.
 * Contributor(s): H.Hokawa <hhokawa @nospam@ gmail.com>.
 ********************************************************************************/
 
$mod_strings = Array(
'LBL_MODULE_NAME'=>'見込み客',
'LBL_DIRECT_REPORTS_FORM_NAME'=>'直接レポート',
'LBL_MODULE_TITLE'=>'見込み客： ホーム',
'LBL_SEARCH_FORM_TITLE'=>'見込み客の検索',
'LBL_LIST_FORM_TITLE'=>'見込み客の一覧',
'LBL_NEW_FORM_TITLE'=>'新規見込み客',
'LBL_LEAD_OPP_FORM_TITLE'=>'顧客担当者-商談：',
'LBL_LEAD'=>'見込み客：',
'LBL_ADDRESS_INFORMATION'=>'住所情報',
'LBL_CUSTOM_INFORMATION'=>'カスタム情報',

'LBL_LIST_NAME'=>'名前',
'LBL_LIST_LAST_NAME'=>'姓',
'LBL_LIST_COMPANY'=>'会社名',
'LBL_LIST_WEBSITE'=>'Web サイト',
'LBL_LIST_LEAD_NAME'=>'見込み客名',
'LBL_LIST_EMAIL'=>'電子メール',
'LBL_LIST_PHONE'=>'電話',
'LBL_LIST_LEAD_ROLE'=>'役割',

'LBL_NAME'=>'名前：',
'LBL_LEAD_NAME'=>'見込み客名：',
'LBL_LEAD_INFORMATION'=>'見込み客情報',
'LBL_FIRST_NAME'=>'名：',
'LBL_PHONE'=>'電話：',
'LBL_COMPANY'=>'会社名：',
'LBL_DESIGNATION'=>'肩書：',
'LBL_PHONE'=>'電話：',
'LBL_LAST_NAME'=>'姓：',
'LBL_MOBILE'=>'携帯電話：',
'LBL_EMAIL'=>'電子メール：',
'LBL_LEAD_SOURCE'=>'紹介元：',
'LBL_LEAD_STATUS'=>'見込み客ステータス：',
'LBL_WEBSITE'=>'Web サイト：',
'LBL_FAX'=>'FAX：',
'LBL_INDUSTRY'=>'業界：',
'LBL_ANNUAL_REVENUE'=>'年間売上高：',
'LBL_RATING'=>'格付け：',
'LBL_LICENSE_KEY'=>'ライセンス キー：',
'LBL_NO_OF_EMPLOYEES'=>'従業員数：',
'LBL_YAHOO_ID'=>'Yahoo! ID：',

'LBL_ADDRESS_STREET'=>'番地：',
'LBL_ADDRESS_POSTAL_CODE'=>'郵便番号：',
'LBL_ADDRESS_CITY'=>'市町村区：',
'LBL_ADDRESS_COUNTRY'=>'国：',
'LBL_ADDRESS_STATE'=>'都道府県：',
'LBL_ADDRESS'=>'住所：',
'LBL_DESCRIPTION_INFORMATION'=>'詳細情報',
'LBL_DESCRIPTION'=>'説明：',

'LBL_CONVERT_LEAD'=>'顧客へ昇格：',
'LBL_CONVERT_LEAD_INFORMATION'=>'顧客へ昇格',
'LBL_ACCOUNT_NAME'=>'顧客企業名',
'LBL_POTENTIAL_NAME'=>'案件名',
'LBL_POTENTIAL_CLOSE_DATE'=>'案件完了日',
'LBL_POTENTIAL_AMOUNT'=>'案件金額',
'LBL_POTENTIAL_SALES_STAGE'=>'案件営業ステージ',


'NTC_DELETE_CONFIRMATION'=>'このレコードを削除しますか？',
'NTC_REMOVE_CONFIRMATION'=>'このケースからこの顧客担当者を削除しますか？',
'NTC_REMOVE_DIRECT_REPORT_CONFIRMATION'=>'$$$Are you sure you want to remove this record as a member organization?',
'NTC_REMOVE_OPP_CONFIRMATION'=>'この商談からこの顧客担当者を削除しますか？',
'ERR_DELETE_RECORD'=>"顧客担当者を削除するには、レコード番号を指定する必要があります。",
'NTC_COPY_PRIMARY_ADDRESS'=>'主要住所を代替住所へコピー',
'NTC_COPY_ALTERNATE_ADDRESS'=>'代替住所を主要住所へコピー',

// Added vtiger_fields in fetchfile.php, import.php -- crm4-Beta
'LBL_COLON'=>' : ', 
'LBL_IMPORT_LEADS'=>'見込み客のインポート',
'LBL_LEADS_FILE_LIST'=>'見込み客ファイル リスト',
'LBL_INSTRUCTIONS'=>'手順',
'LBL_KINDLY_PROVIDE_AN_XLS_FILE'=>'.xls ファイルのみを入力してください',
'LBL_PROVIDE_ATLEAST_ONE_FILE'=>'少なくとも一つのファイルを入力してください',

'LBL_NONE'=>'--なし--',
'LBL_ASSIGNED_TO'=>'担当：',
'LBL_SELECT_LEAD'=>'見込み客の選択',
'LBL_GENERAL_INFORMATION'=>'一般情報',
'LBL_DO_NOT_CREATE_NEW_POTENTIAL'=>'昇格時に新規案件を作成しない',

//for v4 release added
'LBL_NEW_POTENTIAL'=>'新規案件',
'LBL_POTENTIAL_TITLE'=>'案件',

'LBL_NEW_TASK'=>'新規作業',
'LBL_TASK_TITLE'=>'作業',
'LBL_NEW_CALL'=>'新規連絡',
'LBL_CALL_TITLE'=>'連絡',
'LBL_NEW_MEETING'=>'新規会議',
'LBL_MEETING_TITLE'=>'会議',
'LBL_NEW_EMAIL'=>'新規電子メール',
'LBL_EMAIL_TITLE'=>'電子メール',
'LBL_NEW_NOTE'=>'新規ドキュメント',
'LBL_NOTE_TITLE'=>'ドキュメント',
'LBL_NEW_ATTACHMENT'=>'新規添付ファイル',
'LBL_ATTACHMENT_TITLE'=>'添付ファイル',


//Added vtiger_fields after RC1 - Release 
'LBL_ALL'=>'すべて',
'LBL_CONTACTED'=>'接触済み',
'LBL_LOST'=>'消滅',
'LBL_HOT'=>'高有望度 (Hot)',
'LBL_COLD'=>'低有望度 (Cold)',

// Added for 4GA
'LBL_TOOL_FORM_TITLE'=>'見込み客ツール',

'LBL_SELECT_TEMPLATE_TO_MAIL_MERGE'=>'メール マージ テンプレートの選択：',

//Added for 4GA
'Salutation'=>'敬称',
'First Name'=>'名',
'Phone'=>'電話',
'Last Name'=>'姓',
'Mobile'=>'携帯電話',
'Company'=>'会社名',
'Fax'=>'FAX',
'Email'=>'電子メール',
'Lead Source'=>'紹介元',
'Website'=>'Web サイト',
'Annual Revenue'=>'年間売上高',
'Lead Status'=>'見込み客ステータス',
'Industry'=>'業界',
'Rating'=>'格付け',
'No Of Employees'=>'従業員数',
'Assigned To'=>'担当',
'Yahoo Id'=>'Yahoo ID',
'Created Time'=>'作成日時',
'Modified Time'=>'更新日時',
'Street'=>'番地',
'Postal Code'=>'郵便番号',
'City'=>'市町村区',
'Country'=>'国',
'State'=>'都道府県',
'Description'=>'説明',
'Po Box'=>'私書箱',
'Campaign Source'=>'提供元の販売活動',
//Added for CustomView 4.2 Release
'Name'=>'名前',
'LBL_NEW_LEADS'=>'私の新規見込み客',

//Added for Existing Picklist Entries

'--None--'=>'--なし--',
'Mr.'=>'Mr. (男性)',
'Ms.'=>'Ms. (女性)',
'Mrs.'=>'Mrs.',
'Dr.'=>'Dr.',
'Prof.'=>'Prof.',

'Acquired'=>'獲得済み',
'Active'=>'有効',
'Market Failed'=>'市場失敗',
'Project Cancelled'=>'プロジェクト撤退',
'Shutdown'=>'操業停止',

'Apparel'=>'アパレル',
'Banking'=>'銀行',
'Biotechnology'=>'バイオ',
'Chemicals'=>'化学',
'Communications'=>'通信',
'Construction'=>'建設',
'Consulting'=>'コンサルティング',
'Education'=>'教育',
'Electronics'=>'電気',
'Energy'=>'エネルギー',
'Engineering'=>'エンジニアリング',
'Entertainment'=>'エンターテイメント',
'Environmental'=>'環境',
'Finance'=>'金融',
'Food & Beverage'=>'飲食',
'Government'=>'政府・自治体',
'Healthcare'=>'ヘルスケア',
'Hospitality'=>'宿泊',
'Insurance'=>'保険',
'Machinery'=>'機械',
'Manufacturing'=>'製造',
'Media'=>'メディア',
'Not For Profit'=>'NPO (非営利団体)',
'Recreation'=>'レクリエーション',
'Retail'=>'小売',
'Shipping'=>'出荷情報',
'Technology'=>'テクノロジー',
'Telecommunications'=>'電気通信',
'Transportation'=>'運輸',
'Utilities'=>'公共施設',
'Other'=>'その他',

'Cold Call'=>'飛び込み',
'Existing Customer'=>'既存顧客',
'Self Generated'=>'自己開拓',
'Employee'=>'従業員',
'Partner'=>'提携先',
'Public Relations'=>'広報活動',
'Direct Mail'=>'ダイレクト メール',
'Conference'=>'会議 (カンファレンス)',
'Trade Show'=>'展示会',
'Web Site'=>'Web サイト',
'Word of mouth'=>'口コミ',

'Attempted to Contact'=>'接触試行中',
'Cold'=>'低有望度 (Cold)',
'Contact in Future'=>'将来接触予定',
'Contacted'=>'接触済み',
'Hot'=>'高有望度 (Hot)',
'Junk Lead'=>'価値無し',
'Lost Lead'=>'消失',
'Not Contacted'=>'未接触',
'Pre Qualified'=>'事前審査通過',
'Qualified'=>'審査通過',
'Warm'=>'中有望度 (Warm)',

// Added/Updated for vtiger CRM 5.0.4

'Designation'=>'タイトル',
     
// Added after 5.0.4 GA

//Module Sequence Numbering
'Lead No'=>'見込み客 No',
// END

'LBL_TRANSFER_RELATED_RECORDS_TO' => '関連レコードを転送：',
);

?>
